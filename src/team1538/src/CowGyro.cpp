...
#include "../include/team1538/CowGyro.h"
#include <cmath>
...
namespace team1538
{
  ...
  void CowGyro::Handle()
  {
    ...
    std::cout << "Gyro is initialized! PID: 0x" << std::hex << ReadPartId()
              << std::endl;
    ...
    while(true)
    {
      ...
      std::this_thread::sleep_for(
              std::chrono::milliseconds((int) ((1.0 / K_READING_RATE) * 1000)));
      ...
      if((e_StatusFlag::VALID_DATA != status) || !errors.empty())
      {
        ...
        std::cerr << "Gyro read failed. Status: " << status << ". Errors: "
                  << errors.size();
  ...
  bool CowGyro::InitializeGyro()
  {
    ...
    {
      std::cerr << "Gyro not in self test: 0x" << std::hex << selfCheckResult
                << std::endl;
      return false;
    }
    ...
    {
      std::cerr << "Gyro self-test didn't include all errors: 0x" << std::hex
                << selfCheckResult << std::endl;
      return false;
    }
    ...
    if(ExtractStatus(selfCheckResult) != SELF_TEST_DATA)
    {
      std::cerr << "Gyro second self test read failed: 0x" << std::hex
                << selfCheckResult << std::endl;
      return false;
    }
  ...
  int16_t CowGyro::DoRead(int8_t address)
  {
    ...
    while(true)
    {
      ...
      {
        std::cerr << "Unexpected gyro read response: 0x" << std::hex << result
                  << std::endl;
        continue;
      }
  ...
  int32_t CowGyro::DoTransaction(int32_t command)
  {
    ...
    {
      std::cerr << "Transaction failed with size: " << transactionSize
                << std::endl;
    }
  ...
  void CowGyro::FinalizeCalibration()
  {
    ...
    {
      std::cout << "DO NOT have enough zeoring samples, got " << std::dec
                << index << std::endl;
    }
