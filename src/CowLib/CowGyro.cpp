...
namespace CowLib
{
  ...
  //m_RingBuffer(rngCreate(sizeof(st_Accumulation) * COWGYRO_RING_SIZE)),
          m_CircularBuffer(new CowCircularBuffer(
                  sizeof(st_Accumulation) * COWGYRO_RING_SIZE,
                  true)),
          m_Analog(new AnalogInput(channel)),
          m_VoltsPerDegreePerSecond(
                  AnalogGyro::kDefaultVoltsPerDegreePerSecond),
          m_Center(0),
          m_Offset(0),
          m_LastSaveTime(0),
          m_RecalQueued(true)
  {
    ...
    m_Analog->SetAverageBits(AnalogGyro::kAverageBits);
    m_Analog->SetOversampleBits(AnalogGyro::kOversampleBits);
    double sampleRate = AnalogGyro::kSamplesPerSecond *
                        (1 << (AnalogGyro::kAverageBits +
                               AnalogGyro::kOversampleBits));
  ...
  void CowGyro::FinalizeCalibration()
  {
    ...
    m_CircularBuffer->GetBuffer(&acc,
                                sizeof(st_Accumulation) * COWGYRO_RING_SIZE);
    ...
    double average =
            ((double) totalValue / (double) totalCount) + (double) m_Center -
            m_Offset;
